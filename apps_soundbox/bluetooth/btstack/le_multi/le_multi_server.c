...
#define LE_MULTI_S_DEBUG_EN
#ifdef LE_MULTI_S_DEBUG_EN
#define le_multi_s_printf	printf
#define le_multi_s_puts		puts
#define le_multi_s_put_buf	put_buf
#else
#define le_multi_s_printf(...)
#define le_multi_s_puts(...)
#define le_multi_s_put_buf(...)
#endif// LE_MULTI_S_DEBUG_EN



static const uint8_t le_multi_profile_data[] = {
...
#define ATT_CHARACTERISTIC_ae02_01_VALUE_HANDLE 0x000b//chl 0 handle 值
...
#define ATT_CHARACTERISTIC_ae21_01_VALUE_HANDLE 0x0010//chl 1 handle 值
...
static u8 ble_server_init_ok = 0;
...
//        le_multi_s_printf("recieve all: %d\n", test_recieve_all_count);
//    }
//
//}
...
static u32 server_data_send(u8 *data, u16 len)
{
    ...
    if (server_send_handle == (uint16_t)NULL) {
        // not connect
        return 4;//
    }
...
static void server_data_recieve(u8 *data, u16 len)
{
    ...
    le_multi_s_printf("\n server_rdata(%d):", len);
    le_multi_s_put_buf(data, len);
}

//ble send data
static u32 server_data_send1(u8 *data, u16 len)
{
    int ret_val;

    if (server_tone_handle == (uint16_t)NULL) {
        // not connect
        le_multi_s_printf("fun = %s, line = %d\n", __FUNCTION__, __LINE__);
        return 4;//
    }
...
static void app_server_le_msg_callback(uint16_t msg, uint8_t *buffer, uint16_t buffer_size)
{
    /* le_multi_s_printf("\n-%s, msg= 0x%x\n",__FUNCTION__,msg); */
    switch (msg) {
    ...
    case BTSTACK_LE_MSG_INIT_COMPLETE:
        ble_server_init_ok = 1;
        /* le_multi_s_puts("init server_adv\n"); */
        /* server_advertise_enable(); */
        break;
    case BTSTACK_LE_MSG_CONNECT_FAIL:
        le_multi_s_puts("server_conn_fail\n");
        le_multi_server_creat_connect();
        break;
    default:
        break;
    }

    /* le_multi_s_puts("exit_le_msg_callback\n"); */
}
...
// @param buffer_size
// @return 0
static uint16_t app_server_write_callback(uint16_t attribute_handle, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    if (buffer_size > 0) {
        ...
        le_multi_s_put_buf(buffer, buffer_size);
    }
#endif
...
static void app_set_adv_parm(void)
{
...
static void app_set_connect_param(void)
{
// setup connect
...
extern u8 interact_server_set_update_param_st(u8 flag);
static void app_server_conn_update_callback(uint16_t min, uint16_t max, uint16_t timeout)
{
    interact_server_set_update_param_st(0);
    le_multi_s_printf("app_min = %d\n", min);
    le_multi_s_printf("app_max = %d\n", max);
    le_multi_s_printf("timeout= %d\n", timeout);
}
...
{
    le_multi_s_printf("\n%s\n", __FUNCTION__);
    server_register_profile_data(le_multi_profile_data);
 ...
 */
static void ble_enter_mutex(void)
{
    ...
    le_multi_s_puts("in enter mutex\n");
    ble_mutex_flag = 1;
    if (ble_conn_handle != 0) {
    ...
    le_multi_s_puts("exit_enter_mu\n");
}

static void ble_exit_mutex(void)
{
    ...
    le_multi_s_puts("in_exit mutex\n");
    ble_mutex_flag = 0;
    server_advertise_enable();
    le_multi_s_puts("exit_exit_mu\n");
}

static void ble_server_disconnect(void)
{
    if (ble_conn_handle != 0) {
        le_multi_s_printf("server discon handle= 0x%x\n ", ble_conn_handle);
        ble_hci_disconnect(ble_conn_handle);
    }
    le_multi_s_puts("exit_discnt\n");
}


u32 le_multi_server_check_connect_complete(void)
{
    /* le_multi_s_printf("sble_conn_handle = %d, erver_send_handle = %d, server_tone_handle = %d\n", ble_conn_handle, server_send_handle, server_tone_handle); */
...
extern void le_server_request_connection_parameter_update(uint16_t interval_min, uint16_t interval_max, uint16_t conn_latency, uint16_t supervision_timeout);
...
{
    le_multi_s_printf("interval_min = %d, interval_max = %d, conn_latency = %d,  supervision_timeout = %d \n", interval_min, interval_max,  conn_latency, supervision_timeout);
    le_server_request_connection_parameter_update(interval_min, interval_max, conn_latency, supervision_timeout);
}
