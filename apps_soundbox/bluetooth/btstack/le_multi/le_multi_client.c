...
/* #define LE_MULTI_C_DEBUG_EN */
#ifdef LE_MULTI_C_DEBUG_EN
#define le_multi_c_printf		printf
#define le_multi_c_puts			puts
#define le_multi_c_put_buf		put_buf
#else
#define le_multi_c_printf(...)
#define le_multi_c_puts(...)
#define le_multi_c_put_buf(...)
#endif
...
static u16 acc_chr_write_handle = 0;
static u16 acc_chr_notify_handle = 0;

static u16 acc_chr_write_handle2 = 0;
static u16 acc_chr_notify_handle2 = 0;

static u32 client_debug_all;
static u8 ble_client_init_ok = 0;
...
u32 client_data_send(u8 *data, u16 len)
{
    ...
    if (acc_chr_write_handle == (u16)NULL) {
        // not connect
        return 4;//
    }
...
u32 client_data_send1(u8 *data, u16 len)
{
    ...
    if (acc_chr_write_handle2 == (u16)NULL) {
        // not connect
        return 4;//
    }
...
static void parse_report_att_data(att_data_report_t *report)
{
    ...
    /* le_multi_c_printf("value_handle,value_offset: %x,%x",report->value_handle,report->value_offset); */
    ...
    /* le_multi_c_put_buf(report->blob,report->blob_length); */
...
static void app_client_packet_handler(u16 event_type, u16 channel, u8 *packet, u16 size)
{
    ...
    case PK_CLI_SEARCH_SERVICES_COMPLETE:
        ...
        le_multi_c_puts("PK_CLI_SEARCH_SERVICES_COMPLETE\n");
        test_record_timer = get_sys_2msCount();
...
static void client_report_adv_data(adv_report_t *report_pt, u16 len)
{
    ...
    /* le_multi_c_printf("event_type,addr_type: %x,%x; ",report_pt->event_type,report_pt->address_type); */
    /* le_multi_c_put_buf(report_pt->address,6); */
    /* le_multi_c_puts("adv_data_display:"); */
    /* le_multi_c_put_buf(adv_pt->data,adv_pt->length); */

    adv_data_pt = report_pt->data;
    for (i = 0; i < report_pt->length;) {
        if (*adv_data_pt == 0) {
            le_multi_c_puts("analyze end\n");
            break;
        }
        ...
        case 8:
        case 9:
            le_multi_c_puts("local name:");
            break;

        case 0xff:
            le_multi_c_puts("Manufacturer data:");
            /* if(memcmp(adv_data_pt,(void*)JL_manufacturer_data,5) == 0) */
            /* { */
            /* }	 */
            break;
        default:
            le_multi_c_puts("unknow ad_type:");
            break;
        }
        le_multi_c_put_buf(adv_data_pt, lenght - 1);
        adv_data_pt += (lenght - 1);
    }

    if (create_conn_flag) {
        le_multi_c_puts("******creat_connection...\n");
        create_conn_flag = 0;
...
static void app_client_le_msg_callback(u16 msg, u8 *buffer, u16 buffer_size)
{
    ...
    case BTSTACK_LE_MSG_CONNECT_COMPLETE:
        ...
        le_multi_c_printf("conn_handle= 0x%04x\n", client_conn_handle);
    ...
    case BTSTACK_LE_MSG_DISCONNECT_COMPLETE:
        le_multi_c_printf("disconn_handle= 0x%04x\n", buffer[0] + (buffer[1] << 8));
    ...
    case BTSTACK_LE_MSG_INIT_COMPLETE:
        ble_client_init_ok = 1;
        le_multi_set_scan_param(64, 16);
    ...
    case BTSTACK_LE_MSG_CONNECT_FAIL:
        le_multi_c_puts("conn_fail\n");
        le_multi_client_creat_connect();
        break;
...
void le_multi_client_send_test_key_num(u8 key_num)
{
...
void ble_client_test_process(void)
{
    ...
    case 0:
        ...
        break;
    case 1:
        if (le_multi_client_check_connect_complete()) {
