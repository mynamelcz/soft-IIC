...
/* void del_fm_channel(u8 chl_num); */
/* void save_fm_channel(u16 save_freq); */
...
/*----------------------------------------------------------------------------*/
#if FM_REC_EN
bool fm_rec_msg_filter(RECORD_OP_API *rec_fm_api, int msg)
{
    if (rec_fm_api) {
...
*/
/*----------------------------------------------------------------------------*/
bool fm_msg_filter(int msg)
{
    ///FM 任何情况，都必须响应SYS_EVENT_DEL_TASK消息
    //printf("fm_mode_var->scan_mode = %d\n", fm_mode_var->scan_mode);
...
//    notice_player_play_by_path(FM_TASK_NAME, BPF_RADIO_MP3, NULL, NULL);
//    while (1) {
...
//
//    if (OS_NO_ERR == err) {
//        key_msg_reg(FM_TASK_NAME, &fm_mode_key);
