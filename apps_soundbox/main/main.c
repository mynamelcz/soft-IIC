...
#else /*TOYS_EN*/
extern u32 lg_dev_status_chg(u32 status, void *parm, u32 lgdevnum, u8 max_hdl);
#endif	/*TOYS_EN*/
...
extern void set_update_file_name(char *name);
...
#if (BT_BACKMODE == 0 && (BLE_BREDR_MODE&BT_BREDR_EN) == 0 && BLE_MULTI_EN == 1)
#if (BLE_MULTI_GATT_ROLE_CFG == BLE_MULTI_GATT_ROLE_SERVER)
    interact_s_task_creat();
#else
    interact_c_task_creat();
#endif//(BLE_MULTI_GATT_ROLE_CFG == BLE_MULTI_GATT_ROLE_SERVER)
#endif//(BT_BACKMODE == 0 && (BLE_BREDR_MODE&BT_BREDR_EN) == 0 && BLE_MULTI_EN == 1)

    /* #if VR_SPI0_EN */
    /* #define FLASH_VM_START		app_use_flash_cfg.vmif_addr */
    /* #define SPIFAT0_LEN     (1*1024L*1024L) */
    /* #define SPIFAT0_START   (FLASH_VM_START-SPIFAT0_LEN) */
    /* set_spifat0_cap(SPIFAT0_START, SPIFAT0_LEN); */
    /* #endif */

#if VR_SPI1_EN
#define SPIFAT1_LEN     (1*1024L*1024L)
#define SPIFAT1_START   ((2*1024L*1024L)-SPIFAT1_LEN)
    set_spifat1_cap(SPIFAT1_START, SPIFAT1_LEN);
#endif
    ...
    while (1) {
        ...
        case SYS_EVENT_LGDEV_ONLINE:
...
#else /*TOYS_EN*/
            status = lg_dev_status_chg(SYS_EVENT_LGDEV_ONLINE, (void *)msg[1], msg[2], 3);
#endif /*TOYS_EN*/
            ...
            if ((status != FILE_OP_ERR_LGDEV_FULL)/*&&(status!='A')*/) {
                if (((char)status == 'A')) {    ///< SPIFLASH璁惧mount
                    ...
                    puts("*************Select Task***************\n");
                    script_switch(SCRIPT_ID_BT, 0);
                    /* script_switch(SCRIPT_ID_TEST_SEL_DEMO, 0); */
...
#endif
#if(SCRIPT_MUSIC_EN)
                    if (script_get_id() == SCRIPT_ID_MUSIC) {
                        printf("script music has run...\n");
                        if (keymsg_task_name) {
                            os_taskq_post_event((char *)keymsg_task_name, 2, SYS_EVENT_DEV_ONLINE, status);
                        }
                    } else {
                        script_switch(SCRIPT_ID_MUSIC, (void *)status);
                    }
#endif
                }
            }
        ...
        case SYS_EVENT_LGDEV_OFFLINE:
            printf("--------------------------dev offline ---------------------------------------------\n");
            /* if (script_get_id() == SCRIPT_ID_MUSIC || script_get_id() == SCRIPT_ID_REC) { */
            if (keymsg_task_name) {
                os_taskq_post_event((char *)keymsg_task_name, 2, SYS_EVENT_DEV_OFFLINE, msg[1]);
                os_taskq_post_event((char *)NOTICE_PLAYER_TASK_NAME, 2, SYS_EVENT_DEV_OFFLINE, msg[1]);
            }
...
#else /*TOYS_EN*/
            lg_dev_status_chg(SYS_EVENT_LGDEV_OFFLINE, (void *)msg[1], 1, 0);
#endif /*TOYS_EN*/
            break;
        case SYS_EVENT_DEV_BEGIN_MOUNT:
            ...
            if (script_get_id() == SCRIPT_ID_MUSIC) {
                if (keymsg_task_name) {
                    os_taskq_post_event((char *)keymsg_task_name, 1, SYS_EVENT_DEV_BEGIN_MOUNT);
                }
            }
#endif
            break;
        case SYS_EVENT_DEV_MOUNT_ERR:
            ...
            if (script_get_id() == SCRIPT_ID_MUSIC) {
                if (keymsg_task_name) {
                    os_taskq_post_event((char *)keymsg_task_name, 1, SYS_EVENT_DEV_MOUNT_ERR);
                }
            }
#endif
            break;
...
#if(SCRIPT_USB_EN)
            if (script_get_id() == SCRIPT_ID_USB) {
                script_switch(SCRIPT_ID_NEXT, 0);
            }
#endif
        ...
        case SYS_EVENT_AUX_IN:
            ...
            if (os_time_get() > 200) {
                script_switch(SCRIPT_ID_LINEIN, 0);
            }
            break;
        ...
        case MSG_CHANGE_WORKMODE:
            script_switch(SCRIPT_ID_NEXT, 0);
            /* if (1) { */
            /* SCRIPT_ID_TYPE id = script_get_id(); */
            /* id++; */
            /* if (id >= SCRIPT_ID_MAX) { */
            /* id = 0; */
            /* } */
            /* script_switch(id, NULL); */
            /* } */
            break;
        case SYS_EVENT_TASK_RUN_REQ:
            /* task_switch((char *)msg[1], 0,SWITCH_SPEC_TASK); */
#if(SCRIPT_BT_EN)
            if (((u32)msg[1]) >= 0x0001000000) {
                if (0 == strcmp((const char *)msg[1], "BtStackTask")) {
                    puts("run bt stack task !!!\n");
                    script_switch(SCRIPT_ID_BT, 0);
                }
                break;
            }
            printf("SYS_EVENT_TASK_RUN_REQ script_id = %d, line = %d, func = %s\n", (u32)msg[1], __LINE__, __func__);
            script_switch((SCRIPT_ID_TYPE)msg[1], 0);
#endif
            break;
        case MSG_MUTE:
            puts("MSG_MUTE\n");
        ...
        case MSG_POWER_OFF_KEY_MSG:
            ...
            do {
                os_time_dly(1);
            } while (notice_player_get_status() != 0);
...
int main_task_init(void)
{
    ...
    if (OS_NO_ERR == err) {
        key_msg_reg(MAIN_TASK_NAME, &main_mode_key);
