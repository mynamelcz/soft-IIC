...
/* u32 rcsp_cbk_get_decode_time(void) */
/* { */
/* return 0; */
/* } */
...
static u32 cbk_request_frame_deal(u8 *data, u16 len)
{
    ...
    for (cnt = 0; cnt < len; cnt += 4) {
        ...
        case JL_FRAME_ID_SATT:
            tmp32 = __music_player_get_play_time(music_operate_mapi);//rcsp_cbk_get_decode_time;//get_decode_time();
        ...
        case JL_FRAME_ID_EQCU:
            if (rcsp_eq_sw_flag == 0) {
                ...
                if (0 == strcmp(RDEF_BTSTACK_TASK_NAME, get_rcsp_curr_script_name())) {
...
//        return RCSP_CBK_ERR_NONE;
    ...
    printf("next task name = %s\n", next_task_name);
    if (0 == strcmp(next_task_name, get_rcsp_curr_script_name())) {
        //resend frame
        printf("in case, no switch!!\n");
        rcsp_app_start(next_task_name);
    } else {
        u32 sid = get_rcsp_curr_script_id(next_task_name);
        printf("not in case, switch!!\n");
        os_taskq_post(RDEF_MAIN_TASK_NAME, 2, SYS_EVENT_TASK_RUN_REQ, sid);
    }
...
u32 rcsp_music_callback(rcsp_cbk_msg_e pkg_type, u8 *data, u16 len)
{
    ...
    case RCSP_CBK_MSG_MUSIC_SET_BYNO:
        ...
        param_cnt++;
        break;
    case RCSP_CBK_MSG_MUSIC_SPC_FILE:
        msg = MSG_MUSIC_SPC_FILE;
        val_ptr = (u32 *)data;
        printf("spc file = %x\n", *val_ptr);
        param[param_cnt] = (int)(*val_ptr);
        param_cnt++;
        break;
    ...
    if (msg != MSG_NULL) {
        ...
        cbk_post_msg_to_task(SCRIPT_TASK_NAME, msg, param, param_cnt);
    }
