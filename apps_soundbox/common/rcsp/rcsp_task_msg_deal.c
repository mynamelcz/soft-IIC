...
void rcsp_music_init(MUSIC_OP_API *mapi)
{
...
static int is_music_playing(void)
{
    ...
    pp_s = __music_player_get_status(music_operate_mapi);//get_decode_statu(&music_operate_mapi->dop_api->dec_api);
...
void rcsp_music_play_api_after(ENUM_DEV_SEL_MODE dev_sel, u32 dev_let, u32 reslut)
{
    /* P_FUNCTION */
    u8 tmp8;
    if (music_operate_mapi == NULL) {
        return;
    }
    if (reslut == SUCC_MUSIC_START_DEC) {
        printf("rcsp music play after ok, %x\n", music_operate_mapi);
...
{
    char *target_name = SCRIPT_TASK_NAME;//RDEF_LINEIN_TASK_NAME;
    P_FUNCTION
    switch (msg[0]) {
    case RCSP_CBK_MSG_COMM_PLAY:
...
{
    char *target_name = SCRIPT_TASK_NAME;//RDEF_UDISK_TASK_NAME;
    P_FUNCTION
    switch (msg[0]) {
    case RCSP_CBK_MSG_COMM_PLAY:
        if (is_pc_playing() == 0) {
...
void rcsp_pc_task_sync_vol_init(void)
{
    sync_vol_do = 0;
}
