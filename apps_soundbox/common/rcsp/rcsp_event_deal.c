...
#endif
    const char *cur_task_name = get_rcsp_curr_script_name();
    rcsp_apptask_start_deal_do(cur_task_name);

    rcsp_connect_current_task((void *)cur_task_name);
...
void rcsp_apptask_start_deal_do(const char *name)
{
    ...
    P_FUNCTION
    /* task_name = (void *)name; */
    task_name = (void *)get_rcsp_curr_script_name();
...
void rcsp_connect_current_task(char *task_name)
{
    ...
    if (0 == strcmp(task_name, RDEF_MUSIC_TASK_NAME)) {
        ...
        rcsp_report_status(RCSP_REPORT_START_CONFIG);
    } else if (0 == strcmp(cur_task_name, RDEF_BTSTACK_TASK_NAME)) {
        ;
    } else if (0 == strcmp(cur_task_name, RDEF_LIGHT_TASK_NAME)) {
        ;
    } else {
        ;
    }
...
void check_appstart_state_to_rcsp(void)
{
    ...
    next_task_name = (void *)get_rcsp_curr_script_name();//(void *)curr_task->name;
...
void Post_Common_FrameStatus_to_rcsp(void)
{
    ...
    if (0 == strcmp(get_rcsp_curr_script_name(), RDEF_UDISK_TASK_NAME)) {
...
void Post_Mode_EQ_config_to_rcsp(void)
{
    ...
    if (0 == strcmp(get_rcsp_curr_script_name(), RDEF_BTSTACK_TASK_NAME)) {
...
void Post_Mode_EQ_select_to_rcsp(void)
{
    ...
    const char *curr_task_name = get_rcsp_curr_script_name();
    if (0 == strcmp(curr_task_name, RDEF_BTSTACK_TASK_NAME)) {
        value[0] = sysEQ_to_rcspEQ_talbe[get_bt_eq_mode()];
    } else if (0 == strcmp(curr_task_name, RDEF_MUSIC_TASK_NAME)) {
        value[0] = sysEQ_to_rcspEQ_talbe[eq_mode];
    } else if (0 == strcmp(curr_task_name, RDEF_RADIO_TASK_NAME)) {
        value[0] = sysEQ_to_rcspEQ_talbe[eq_mode];
    } else {
        return;
    }
...
//    printf("music_file,cur_num= %d,total_num= %d \n",music_cur_filenum,music_cur_filetotal);
    ...
    tmp32 =  __music_player_get_play_time(music_operate_mapi);//get_decode_time();
...
void Post_Music_StartInfo_to_rcsp(void)
{
    ...
    rcsp_db_printf("music_time: start= %d,curr= %d, total= %d, %d, %s\n", 0, tmp32, music_playfile_total_time, __LINE__, __func__);
