...
const SCRIPT_ID_TYPE support_taskid_group[TASK_APP_MAX] = {
    SCRIPT_ID_BT,
    SCRIPT_ID_MUSIC,
#ifdef TASK_RADIO_APP
    SCRIPT_ID_FM,
...
const char *get_rcsp_curr_script_name(void)
{
    SCRIPT_ID_TYPE sid = script_get_id();
    u32 i;
    for (i = 0; i < (sizeof(support_taskid_group) / sizeof(support_taskid_group[0])); i++) {
        if (sid == support_taskid_group[i]) {
            return support_taskname_group[i];
        }
    }
    return NULL;
}

u32 get_rcsp_curr_script_id(const char *rcsp_script_name)
{
    u32 i;
    for (i = 0; i < (sizeof(support_taskname_group) / sizeof(support_taskname_group[0])); i++) {
        if (0 == strcmp(rcsp_script_name, support_taskname_group[i])) {
            return (u32)support_taskid_group[i];
        }
    }
    return (u32)SCRIPT_ID_UNACTIVE;
}
 ...
 *********************************************************************************************************
 */
extern void rscp_set_fs_op_msg_cbk(u32(*cbk)(rcsp_cbk_msg_e pkg_type, u8 *data, u16 len));
...
static u32 music_subcmd_deal(u8 *OperationData, u8 *data, u16 len)
{
    ...
    case 0xfe:
        // set path
        rscp_set_fs_op_msg_cbk(support_app_msg_callback_group[rcsp_curr_task_id]);
