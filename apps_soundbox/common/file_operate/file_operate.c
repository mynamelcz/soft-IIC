...
#define SAVE_BREAKPOINT_MAX_DEV		(3) ///注意：此处决定记忆断点的设备数量，必须根据vm枚举中断点定义一致，3表示U盘、sd0、sd1, 否则vm记忆出错
...
extern u32 lg_dev_open_file_by_flash_rec_addr(void *parm, u32 flash_addr);
...
static u32 file_operate_op(u32 cmd, FILE_OPERATE *fop, void *input, void *output)
{
    ...
    case FOP_OPEN_FILE_BYSCLUCT:    //指定簇号播放，用于
        ...
        break;
#ifdef TOYS_EN
    case FOP_OPEN_FILE_BY_FLASH_REC_ADDR:
        status = lg_dev_open_file_by_flash_rec_addr(cur_fop->cur_lgdev_info, *((u32 *)input));
        //printf("status = 0x%x \n", status);
        if (status == true) {
            /* cur_fop->cur_lgdev_info->last_op_file_num = *((u32 *)input); */
            status = FILE_OP_NO_ERR;
...
//                file_op_deg("open file num = %08x   \n",*tmp);
    ...
    case FOP_READ_BPINFO:
        ...
        if (status < SAVE_BREAKPOINT_MAX_DEV) {
            FS_BRK_POINT *tmp_brk = (FS_BRK_POINT *)output;
            FS_BRK_INFO *tmpbp = &(tmp_brk->inf);
            if ((tmp_brk->vm_base_index == 0) || (tmp_brk->vm_base_flac_index == 0)) {
                file_op_deg("without vm index, ignore~~~ %d %s!!!\n", __LINE__, __func__);
                status = FILE_OP_NO_ERR;
                break;
            }
            status += tmp_brk->vm_base_index;
            file_op_deg("bp read =%08x \n", status);
            status = vm_read_api(status, tmpbp);
            if (status == VM_FILE_BK_LEN) {
                u8 i;
                for (i = 0; i < VM_FILE_BK_LEN; i++) {
                    file_op_deg("%02x ", *((u8 *)tmpbp + i));
                }
...
//                            printf_buf(tmpbp,VM_FILE_BK_LEN);
                ...
                {file_op_deg("bp crc err \n");
                    memset((u8*)tmpbp,0,VM_FILE_BK_LEN);
    ...
    case FOP_READ_FLACBPINFO:
        ...
        if (status < SAVE_BREAKPOINT_MAX_DEV) {
            ...
            status += tmp_brk->vm_base_flac_index;
            file_op_deg("bp read =%08x \n", status);
            status = vm_read_api(status, ((FS_BRK_INFO *)tmpbp)->flac_brk_buff);
            if (status == VM_FILE_FLACBK_LEN) {
                u16 i;
                for (i = 0; i < VM_FILE_FLACBK_LEN; i++) {
                    file_op_deg("%02x ", *((u8 *)((((FS_BRK_INFO *)tmpbp)->flac_brk_buff)) + i));
                }
                file_op_deg("bp read ok \n");
                /* crcvalue=CRC16((u8*)tmpbp,VM_FILE_BK_LEN-2); */
                /* if(crcvalue==tmpbp->crc) */
...
//                            printf_buf(tmpbp,VM_FILE_BK_LEN);
                ...
                if (status > VM_FILE_BK_LEN) {
                    file_op_deg("bp read err1 \n");
                    memset((u8 *)(((FS_BRK_INFO *)tmpbp)->flac_brk_buff), 0, VM_FILE_BK_LEN);
                }
                status = FILE_OP_ERR_OPEN_FILE;
            }
        } else {
    ...
    case FOP_SAVE_BPINFO:
        ...
        if (status < SAVE_BREAKPOINT_MAX_DEV) {
            ...
            FS_BRK_POINT *tmp_brk = (FS_BRK_POINT *)input;
            FS_BRK_INFO *tmpbp = &(tmp_brk->inf);
            if ((tmp_brk->vm_base_index == 0) || (tmp_brk->vm_base_flac_index == 0)) {
                file_op_deg("without vm index, ignore~~~ %d %s!!!\n", __LINE__, __func__);
                status = FILE_OP_NO_ERR;
            ...
            if (fs_get_brk_info(tmp_brk, cur_fop->cur_lgdev_info->lg_hdl->file_hdl)) {
                file_op_deg("bp save ge finfo ok \n");
                file_op_deg("vm index = %d, vm flac index = %d \n", tmp_brk->vm_base_index, tmp_brk->vm_base_flac_index);
                /* FS_BRK_INFO *tmpbp; */
                ...
                if (VM_FILE_BK_LEN == vm_write_api(status, tmpbp)) {
    ...
    case FOP_SAVE_FLACBPINFO:
        ...
        if (status < SAVE_BREAKPOINT_MAX_DEV) {
            ...
            if (fs_get_brk_info(tmp_brk, cur_fop->cur_lgdev_info->lg_hdl->file_hdl)) {
                ...
                /* memset(((FS_BRK_INFO *)tmpbp)->norm_brk_buff,1,8); */
                if ((VM_FILE_BK_LEN == vm_write_api(status + tmp_brk->vm_base_index, tmpbp)) && (VM_FILE_FLACBK_LEN == vm_write_api(status + tmp_brk->vm_base_flac_index, ((FS_BRK_INFO *)tmpbp)->flac_brk_buff))) { //
                    file_op_deg("bp save ok \n");
                    //                            printf_buf(tmpbp,VM_FILE_BK_LEN);
                    status = FILE_OP_NO_ERR;
                    break;
    ...
    case FOP_CLEAR_BPINFO:
        ...
        if (status < SAVE_BREAKPOINT_MAX_DEV) {
            ...
            /* for(i=0;i<VM_FILE_BK_LEN;i++) */
            /* file_op_deg("%02x ",*((u8 *)tmpbp+i)); */
            memset((u8 *)tmpbp, 0, sizeof(FS_BRK_INFO));
            if ((VM_FILE_BK_LEN == vm_write_api(status + tmp_brk->vm_base_index, tmpbp)) && (VM_FILE_FLACBK_LEN == vm_write_api(status + tmp_brk->vm_base_flac_index, ((FS_BRK_INFO *)tmpbp)->flac_brk_buff))) { //
                file_op_deg("bp save ok \n");
                /* printf_buf(tmpbp,VM_FILE_BK_LEN); */
                for (i = 0; i < VM_FILE_BK_LEN; i++) {
                    file_op_deg("%02x ", *((u8 *)((((FS_BRK_INFO *)tmpbp))) + i));
                }
                status = FILE_OP_NO_ERR;
                break;
            } else {
    ...
    case FOP_CLOSE_LOGDEV:
        ...
        lg_dev_close_all();
#else /*TOYS_EN*/
        lg_dev_close(cur_fop->cur_lgdev_info);
        cur_fop->cur_lgdev_info = NULL;
#endif /*TOYS_EN*/
        break;
...
static u32 file_sel_op(u32 cmd, FILE_OPERATE *fop, void *input, void *output)
{
    ...
    case PLAY_SPEC_FILE:
        if (cur_fop->fop_init->cur_play_mode == REPEAT_FOLDER) {
            /* printf("******************************cur file =%d, first file = %d, last_file = %d\n", curr_file, first_file, last_file); */
            if ((curr_file < first_file) || (curr_file > last_file)) {
...
#ifdef TOYS_EN
        else if (PLAY_SEL_FLASH_REC == cmd) {
            status = file_operate_op(FOP_OPEN_FILE_BY_FLASH_REC_ADDR, cur_fop, input, output);
        }
#endif
        else {
