...
const u16 vm_index_len[VM_MAX_SIZE] = {
    ...
    VM_OSC_INT_R_LEN,
    VM_OSC_INT_L_LEN,
    VM_PC_MIC_VOL_LEN, /*PC VOL*/
    /* VM_DEV0_BK_REC_LEN, */
    /* VM_DEV1_BK_REC_LEN, */
    /* VM_DEV2_BK_REC_LEN, */
    /* VM_DEV3_BK_REC_LEN, */
    /* VM_DEV0_FLACBK_REC_LEN, */
    /* VM_DEV1_FLACBK_REC_LEN, */
    /* VM_DEV2_FLACBK_REC_LEN, */
    /* VM_DEV3_FLACBK_REC_LEN, */
    /* MAX_VM,          */
};
...
{
    u32 vm_index = 0;
    vm_index = vm_open(hdl, len);
    u8 err = vm_write((vm_hdl)vm_index, data_buf);
    return VM_ERR_NONE;
}
...
vm_err br17_vm_read(u8 hdl, void *data_buf, int len)
{
    u32 vm_index = 0;
    vm_index = vm_open(hdl, len);
    u8 err = vm_read((vm_hdl)vm_index, data_buf);
    return VM_ERR_NONE;
}
/*----------------------------------------------------------------------------*/
/** @brief: vm test
...
vm_err vm_init_api(struct flash_cfg *cfg)
{
    ...
    tbool err = SetVmIndexNo(VM_INDEX_DES_MAX);
    if (err == false) {
        puts("vm init malloc vm index no err-- \n");
        while (1);
    }
    vm_efuse_key_init();
