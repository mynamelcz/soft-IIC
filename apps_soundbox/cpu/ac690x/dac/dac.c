...
{
#ifdef MIO_USB_DEBUG
    if ((g_mio_var) && (g_mio_var->status & MIO_API_STU_USB_DEG)) {
        mio_api_run((mio_ctrl_t *)g_mio_var);
        return;
    }
#endif
    ...
    if (false == sound_mix_api_process(read_buf, DAC_BUF_LEN)) {
        /* putchar('I'); */
        ///如果全局叠加没有使能，从dac_buf获取数据
        if (0 == dac_read_en) {
            //		putchar('l');
            if (dac_cbuf_enough() != 0)	{
                memset(dac_buf, 0x00, DAC_BUF_LEN);
                //#if SPI_REC_EN
                //            spirec_save_dac(dac_buf);
                //#endif
                ...
                rd_da((s16 *)dac_buf, (PITCH_EN ? 0 : (BIT(7) | 0)));//如果没有使能实时魔音，不获取混响第一声，混响第一声走模拟
#endif//(SOUND_MIX_GLOBAL_EN == 0)
#endif
                return;
            }
            dac_read_en = 1;
...
#if((SCRIPT_MIDI_EN == 1) && (SOUND_MIX_GLOBAL_EN == 0))
    if (script_get_id() == SCRIPT_ID_MIDI) {
        dac_cbuf_write_detect(OUTPUT_BUF_SIZE - 1024);///limit 不能大于cbuf的最大缓存OUTPUT_BUF_SIZE
    } else
#endif
    {
        dac_cbuf_write_detect(DAC_BUF_LEN);///limit 不能大于cbuf的最大缓存OUTPUT_BUF_SIZE
    }
}
#ifdef ENCODE_FLASH_API
AT_ENC_ISR void dac_isr_callback_fuc(void *dac_buf, u8 buf_flag)
{
    if (enc_flash_work) {
        return;
    }
    dac_isr_callback(dac_buf, buf_flag);
}
#endif
...
void audio_init(void)
{
    ...
    ladc_set_use_mode(1);//上电初始化好mic的模拟通道
