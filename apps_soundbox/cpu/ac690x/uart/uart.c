...
#ifndef AT_UART_IO_RAM
#define AT_UART_IO_RAM AT(.uart_io_ram)
#endif
...
///0x3f        USBDM
        if (UART_INPUT_CHAL == uart_updata_param.io) { ///设置要作为串口的GPIO
            ...
            uart_module_ctrl(&uart1_handle, UART_CMD_SEL_INPUT_IO, (u32) JL_PORTA, (void *)BIT(0));
        }
        if (!status) {
            status = uart_module_start(&uart1_handle);
            if (!status) {
...
AT_UART_IO_RAM void putbyte(char a)
{
    if (uart_put_handle) {
        if (a == '\n') {
...
AT_UART_IO_RAM void putchar(char a)
{
    if (a == '\n') {
        putbyte(0x0d);
        putbyte(0x0a);
...
AT_UART_IO_RAM void put_u4hex(u8 dat)
{
    dat = 0xf & dat;
...
AT_UART_IO_RAM void put_u32hex(u32 dat)
{
    putbyte('0');
    putbyte('x');
    put_u4hex(dat >> 28);
...
AT_UART_IO_RAM int puts(const char *out)
{
    while (*out != '\0') {
        putchar(*out);
        out++;
