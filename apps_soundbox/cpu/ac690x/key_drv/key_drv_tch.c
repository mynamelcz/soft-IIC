...
AT_ENC_ISR_TCH_KEY tu8 get_tchkey_value(void)
{
    u8 key;
    u8 i;
...
AT_ENC_ISR_TCH_KEY static void ctm_irq(u8 channel)
{
...
//触摸中断
AT_ENC_ISR_TCH_KEY void ctm_irq_isr(void)
{
    u8 channel;
    CTM_CPND(1);
    channel = CTM_CH();
...
AT_ENC_ISR_TCH_KEY IRQ_REGISTER(IRQ_CTM_IDX, ctm_irq_isr);
