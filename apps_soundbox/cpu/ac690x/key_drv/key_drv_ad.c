...
u16 adc_value[3];
const u32 ad_table[] AT_ENC_ISR_AD_KEY;
const u32 ad_table[] = {
#if KEY_AD_EN
    /* AD_KEY_PA6, */
    AD_KEY_PR2,
...
const u16 ad_key_table[] AT_ENC_ISR_AD_KEY;
...
AT_ENC_ISR_AD_KEY void adc_scan(void *param)
{
    if (adc_busy_flag) {
        return;
    }
...
/**@brief   获取ad按键值
   ...
   @param   void
   @return  key_number
   @note    u32 get_adkey_value(void)
*/
/*----------------------------------------------------------------------------*/
AT_ENC_ISR_AD_KEY tu8 get_adkey_value(void)
{
#if KEY_AD_EN
    tu8 key_number;
    u32 key_value;
...
#endif//(BT_LOW_POWER_MODE != BT_POWER_DOWN_EN)
