...
/* #define ENABLE_EXCEPTION_ISR_DEBUG */
...
#ifdef ENABLE_EXCEPTION_ISR_DEBUG
IRQ_REGISTER(IRQ_EXCEPTION_IDX, exception_isr);
#endif//ENABLE_EXCEPTION_ISR_DEBUG
...
void set_g_sys_cfg(FLASH_SYS_CFG *sys_cfg, u32 cfg_addr)
{
    ...
    for (i = 0; i < app_use_flash_cfg.cfg_zone_size; i += 16) {
        ...
        if (memcmp((const char *)cfg_data, "BTIF", 4) == 0) {
...
#if LCD_128X64_EN
        else if (memcmp((const char *)cfg_data, "LRIF", 4) == 0) { ///lrc解释临时flash保存区域, 在isd_tools.cfg文件中配置
            tmp = (u32 *)(cfg_data + 4);
            tmp_addr = *tmp;
            tmp = (u32 *)(cfg_data + 8);
            tmp_len = *tmp;
            ...
            lrc_label_save_to_flash_init(tmp_addr, tmp_len, sys_cfg->flash_cfg.flash_base);
        }
#endif
#ifdef ENCODE_FLASH_API
        else if (memcmp((const char *)cfg_data, "REIF", 4) == 0) {

            tmp = (u32 *)(cfg_data + 4);
            tmp_addr = *tmp;
            tmp = (u32 *)(cfg_data + 8);
            tmp_len = *tmp;
            printf("ENCODE_FLASH_addr =%08x \n", tmp_addr);
            printf("ENCODE_FLASH_len =%08x \n", tmp_len);
            encode_flash_zone(tmp_addr, tmp_len);
        }
#endif

#if VR_SPI0_EN
        /* set_spifat0_cap(SPIFAT0_START, SPIFAT0_LEN); */
        else if (memcmp((const char *)cfg_data, "F0IF", 4) == 0) {

            tmp = (u32 *)(cfg_data + 4);
            tmp_addr = *tmp;
            tmp = (u32 *)(cfg_data + 8);
            tmp_len = *tmp;
            printf("VR_SPI0_addr =%08x \n", tmp_addr);
            printf("VR_SPI0_len =%08x \n", tmp_len);
            set_spifat0_cap(tmp_addr, tmp_len);
        }
#endif

#ifdef MIO_USB_DEBUG
        else if (memcmp((const char *)cfg_data, "IOIF", 4) == 0) {
            ...
            printf("mio_addr =%08x \n", tmp_addr);
            printf("mio_len =%08x \n", tmp_len);
            music_usb_mio_cap(tmp_addr, tmp_len);
        }
#endif
        else {
            tmp = (u32 *)(cfg_data + 4);
...
typedef struct __startInfo {
    u8 serialNumber[32];
    u32 flag_value;
} _startInfo;
_startInfo startInfo;
void  board_main(u32 cfg_addr, u32 addr, u32 res, _startInfo *info)
{
    u32 tmp;
    ...
    JL_SYSTEM->LVD_CON |= BIT(0);//2.2V
//	JL_SYSTEM->LVD_CON |= BIT(1);//2.4V
//	JL_SYSTEM->LVD_CON |= BIT(1)|BIT(0);//2.6V

    open_wdt(0x0f);			//set wd 32s
    uart_init(UART_BAUD_RAE); //<串口波特率
    memcpy(&startInfo, info, sizeof(_startInfo));
    printf_buf(startInfo.serialNumber, 32);
    printf("update flag:%x \n", startInfo.flag_value);
    update_check(startInfo.flag_value);
    /* printf("SFC_BAUD=%d\n", JL_SFC->BAUD); */
    ...
    dev_cache_set_header_base(sys_cfg.flash_cfg.sdfile_head_addr);
    printf("set_sydf_header_base = %x\n", sys_cfg.flash_cfg.sdfile_head_addr);

    set_cache_base(FLASH_BASE_ADDR);

#if MIDI_UPDATE_EN
    midi_update_set_flash_base_addr(sys_cfg.flash_cfg.flash_base);
#endif
...
#ifdef ENABLE_EXCEPTION_ISR_DEBUG
    irq_handler_register(0, exception_isr, 1);
#endif//ENABLE_EXCEPTION_ISR_DEBUG
