...
typedef struct __USB_SPK_GETLEN {
    void *priv;
    u32(*cbk)(void *priv);
} USB_SPK_GETLEN;

typedef struct __USB_SPK_DATA_STREAM {
    DEC_SET_INFO_CB set_info_cb;
    DEC_OP_CB		op_cb;
    _OP_IO 			output;
    u32 			obuf_size;
    USB_SPK_GETLEN	getlen;
    void 		   *mix;
} USB_SPK_DATA_STREAM;


void usb_speaker_output_before_register(void (*cbk)(USB_SPK_DATA_STREAM *stream, void *priv), void *priv);
void usb_speaker_output_after_register(void (*cbk)(USB_SPK_DATA_STREAM *stream, void *priv), void *priv);
void usb_speaker_output_stop_enable(u8 enable);
