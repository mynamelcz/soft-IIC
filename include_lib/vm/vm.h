...
#ifdef TOYS_EN
bool sfc_erase_no_close_isr(FLASH_ERASER cmd, u32 addr);
#endif /*TOYS_EN*/
...
//设置不检测保护区间（传入了地址是物理地址）
void set_no_protect_area(u32 addr, u32 len);

//加密写
u32 sfc_enc_write(const u8 *buf, u32 addr, u32 len);
void set_vm_efuse_key(u16 key);
//如果使用加密写，就要初始化这个
u16 vm_efuse_key_init(void);

//设置vm index使用的范围,最大255
tbool SetVmIndexNo(u8 IndexNo);
#endif  //_VM_H_
