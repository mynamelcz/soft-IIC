...
typedef  struct _EX_MELODY_STRUCT_ {
    void *priv;
    u32(*melody_trigger)(void *priv, u8 key, u8 vel);//如果key在0~127表示音符开始回调， 如果为0xff表示没有音符按下，并且已经音符播放结束
} EX_MELODY_STRUCT;
...
enum {
    //CMD_MIDI_CTRL_MODE_0        =0X00,
    CMD_MIDI_CTRL_MODE_1          = 0X01,//midi one key one note 模式
    CMD_MIDI_CTRL_MODE_2,				 //midi正常播放模式
    CMD_MIDI_CTRL_MODE_W2S,              //外部音源
    CMD_MIDI_CTRL_MODE_ERR,				 //错误模式
};
...
typedef  struct _MIDI_PLAY_CTRL_TEMPO_ {
    u16 tempo_val;
    u16 decay_val;//default 1024, one key one note 建议范围：970~1000
    u32 mute_threshold;//melody mute threshold 主要是用于判断音符播放结束
