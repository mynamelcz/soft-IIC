...
#define FREE_CAP_COUNT_EN/*剩余空间统计*/
#ifdef FREE_CAP_COUNT_EN

#define F_FREE_FLAG_TOTAL   BIT(0)

#define FREE_FAT_MUTEX_ENTER(fs) //do{if ((fs)->mutex) os_mutex_pend((fs)->mutex,0);}while(0)
#define FREE_FAT_MUTEX_EXIT(fs)  //do{if ((fs)->mutex) os_mutex_post((fs)->mutex);}while(0)
...
#ifdef TOYS_EN
_banked_func void f_add_free_clust_flag(FATFS _xdata *fs, u32 clust);
_banked_func void f_del_free_clust_flag(FATFS _xdata *fs, u32 clust);
#define FREE_CLS_USE_TOTAL_FLAG     0
#define FREE_CLS_RESET_TOTAL_FLAG   1
#define FREE_CLS_RESET_ALL          2
_banked_func u32 f_get_total_free(FATFS _xdata *fs, u32 limit, u32 *cls_size, u8 parm);
_banked_func void f_set_freecls_buf(FATFS _xdata *fs, void *buf, u32 len);
#endif /*TOYS_EN*/
